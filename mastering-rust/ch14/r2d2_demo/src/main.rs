...
fn main() {
    let manager = PostgresConnectionManager::new("postgres://postgres@localhost:5432", TlsMode::None).unwrap();
    ...
    thread::spawn(move || {
        ...
        conn.execute("INSERT INTO books (id, title, author, year) VALUES ($1, $2, $3, $4)", &[&book.id, &book.title, &book.author, &book.year]).unwrap();
    });
